...
void uart_task_code( void* void_parameters ) {
  struct uart_task_parameters* parameters = (struct uart_task_parameters*)void_parameters;
  QueueHandle_t queue_handle = parameters->queue_handle;

  struct uart_message* message;
  INT8U string_integer[16] = {'\0'};
  INT8U string_decimal[16] = {'\0'};
  int i = 0;

  while (1) {
    if (xQueueReceive(queue_handle, message, (TickType_t) 10)) {
      switch (message->message_type) {
        case e_uart_message_pan:
          to_string(message->message_data / 100, string_integer);
          to_string(message->message_data % 100, string_decimal);
          ...
          uart0_putc(',');
          wait_until_ready();
          while (string_integer[i] != '\0') uart0_putc(string_integer[i]);
          i = 0;
          wait_until_ready();
          uart0_putc('.');
          wait_until_ready();
          while (string_decimal[i] != '\0') uart0_putc(string_decimal[i]);
          wait_until_ready();
          uart0_putc(';');
          break;

        case e_uart_message_tilt:
          to_string(message->message_data / 100, string_integer);
          to_string(message->message_data % 100, string_decimal);
...
void uart_task_create(void* parameters) {
  xTaskCreate(uart_task_code, "UART task", STACK_SIZE_UART, parameters, PRIO_UART, (void*)0 );
}
